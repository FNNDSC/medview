...
var viewer = viewer || {};
  ...
  viewer.Viewer.prototype.setVolume = function(nodeObj) {
    ...
    if (nodeObj.data.fileObjs && nodeObj.data.fileObjs.length) {
      ...
      function render() {
        ...
        self.sliceX.add(self.volume);
        // start the loading/rendering
        self.sliceX.render();
      }

      function add(fileObj, pos) {
  ...
  viewer.Viewer.getHTMLTemplate = function(){
    ...
    '<div class="topLeftContainer">' +
        ...
        '</div>' +
        '<details open class="bordered">' +
          '<summary>Select files</summary>' +
          '<div class="content">' +
            '<div class="fileselectwidget">' +
              '<div id="tree"> </div>' +
            '</div>' +
          '</div>' +
        '</details>' +
      ...
      '</div>' +
      '<div class="row-fluid smallRenderers">' +
        '<div class="xSmallRenderer renderer left">' +
          '<div id="sliceZ" class="border-top green"></div>' +
        '</div>' +
        '<div class="smallRenderer renderer center">' +
          '<div id="sliceX" class="border-top red"></div>' +
        '</div>' +
        '<div class="xSmallRenderer renderer right">' +
          '<div id="sliceY" class="border-top blue"></div>' +
        '</div>' +
      '</div>' +
    '</div>';
  }

  // static method to determine if the volume extension is supported
  viewer.Viewer.isVolume = function(filename) {
    // volume extensions
    var ext = ['.mgh', '.mgz', '.nrrd', '.nii', '.nii.gz'];

    return viewer.strEndsWith(filename, ext);
  }

  // static method to determine if the fibers extension is supported
  viewer.Viewer.isFibers = function(filename) {
    // fiber extension is .trk
    var ext = ['.trk'];

    return viewer.strEndsWith(filename, ext);
  }

  // static method to determine if the geometric model extension is supported
  viewer.Viewer.isGeomModel = function(filename) {
  ...
  // static method to determine if the DICOM extension is supported
  viewer.Viewer.isDicom = function(filename) {
    // geometric model extensions
    var ext = ['.dcm', '.ima', '.DCM', '.IMA'];
 ...
 */
  ...
  // returns true if the string str ends with any of the specified suffixes in
  // arrayOfStr otherwise return false
